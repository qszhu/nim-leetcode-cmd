import utils



proc decryptAES128CBC*(input, key, iv: seq[uint8]): seq[uint8]

proc decrypt*(encrypted, key, iv: string): string =
  let encrypted = cast[seq[uint8]](encrypted)
  let key = cast[seq[uint8]](key)
  let iv = cast[seq[uint8]](iv)
  cast[string](decryptAES128CBC(encrypted, key, iv))

# DON'T USE IN PRODUCTION
# http://www.moserware.com/2009/09/stick-figure-guide-to-advanced.html
# https://blog.nindalf.com/posts/implementing-aes/
# https://github.com/CrackedPoly/AES-go
const SBOX: array[256, uint8] = [
  ,
]

proc subBytes(state: var seq[uint8]) =
  for i, v in state:
    state[i] = SBOX[v]

const INV_SBOX: array[256, uint8] = [
  ,
]

proc invSubBytes(state: var seq[uint8]) =
  for i, v in state:
    state[i] = INV_SBOX[v]

const RCON: array[10, uint32] = [
  0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
  0x20000000, 0x40000000, 0x80000000'u32, 0x1b000000, 0x36000000,
]

type
  AESCipher = ref object
    nk: int # state words
    nr: int # rounds
    key: seq[uint8]
    roundKeys: seq[uint32]

proc keyExpansion(self: AESCipher): seq[uint32]
proc newAES(key: seq[uint8]): AESCipher =
  result.new
  result.nk = 4
  result.nr = 10
  result.key = key
  result.roundKeys = result.keyExpansion

proc `xor=`(x: var seq[uint8], y: seq[uint8]) =
  for i in 0 ..< x.len:
    x[i] = x[i] xor y[i]

proc rotWord(b: var seq[uint8]) =
  (b[0], b[1], b[2], b[3]) = (b[1], b[2], b[3], b[0])

proc keyExpansion(self: AESCipher): seq[uint32] =
  result = newSeq[uint32]()
  for i in 0 ..< self.nk:
    result.add toUint32BE(self.key, i * 4)

  for i in self.nk ..< (self.nr + 1) * 4:
    var t = fromUint32BE(result[i - 1])
    if i mod self.nk == 0:
      t.rotWord
      t.subBytes
      var tr = fromUint32BE(RCON[i div self.nk - 1])
      t.`xor=`(tr)
    result.add result[i - self.nk] xor t.toUint32BE

proc addRoundKey(state: var seq[uint8], w: seq[uint32]) =
  var t = newSeq[uint8](16)
  for i in 0 ..< w.len:
    t.fromUint32BE(w[i], i * 4)
  state.`xor=`(t)

proc shiftRow(b: var seq[uint8], i, n: int) =
  (b[i], b[i + 4], b[i + 8], b[i + 12]) = (
    b[i + 4 * (n mod 4)],
    b[i + 4 * ((n + 1) mod 4)],
    b[i + 4 * ((n + 2) mod 4)],
    b[i + 4 * ((n + 3) mod 4)]
  )

proc invShiftRows(state: var seq[uint8]) =
  state.shiftRow(1, 3)
  state.shiftRow(2, 2)
  state.shiftRow(3, 1)

proc xtime(b: uint8): uint8 =
  (b shl 1) xor (((b shr 7) and 1) * 0x1b)

proc xtimes(b: uint8, r: int): uint8 =
  result = b
  for _ in 0 ..< r:
    result = xtime(result)

proc mulByte(x, y: uint8): uint8 {.inline.} =
  (
    (((y shr 0) and 0x01) * xtimes(x, 0)) xor
    (((y shr 1) and 0x01) * xtimes(x, 1)) xor
    (((y shr 2) and 0x01) * xtimes(x, 2)) xor
    (((y shr 3) and 0x01) * xtimes(x, 3)) xor
    (((y shr 4) and 0x01) * xtimes(x, 4)) xor
    (((y shr 5) and 0x01) * xtimes(x, 5)) xor
    (((y shr 6) and 0x01) * xtimes(x, 6)) xor
    (((y shr 7) and 0x01) * xtimes(x, 7))
  )

proc mulWord(state: var seq[uint8], i: int, s: openArray[uint8]) =
  let t = state[i ..< i + 4]
  state[i + 0] = mulByte(t[0], s[3]) xor mulByte(t[1], s[0]) xor mulByte(t[2], s[1]) xor mulByte(t[3], s[2])
  state[i + 1] = mulByte(t[0], s[2]) xor mulByte(t[1], s[3]) xor mulByte(t[2], s[0]) xor mulByte(t[3], s[1])
  state[i + 2] = mulByte(t[0], s[1]) xor mulByte(t[1], s[2]) xor mulByte(t[2], s[3]) xor mulByte(t[3], s[0])
  state[i + 3] = mulByte(t[0], s[0]) xor mulByte(t[1], s[1]) xor mulByte(t[2], s[2]) xor mulByte(t[3], s[3])

proc invMixColumns(state: var seq[uint8]) =
  const s: array[4, uint8] = [0x0b, 0x0d, 0x09, 0x0e]
  for i in countup(0, state.len - 1, 4):
    mulWord(state, i, s)

proc decryptBlock(self: AESCipher, state: var seq[uint8]) =
  addRoundKey(state, self.roundKeys[self.nr * 4 ..< (self.nr + 1) * 4])
  for r in countdown(self.nr - 1, 1):
    invShiftRows(state)
    invSubBytes(state)
    addRoundKey(state, self.roundKeys[r * 4 ..< (r + 1) * 4])
    invMixColumns(state)
  invShiftRows(state)
  invSubBytes(state)
  addRoundKey(state, self.roundKeys[0 ..< 4])

proc unpadPKCS7(input: var seq[uint8]): seq[uint8] =
  let last = input[^1].int
  input[0 ..< ^last]

proc decryptCBC(self: AESCipher, input: var seq[uint8], iv: seq[uint8]): seq[uint8] =
  var iv = iv
  var reg = newSeq[uint8](16)
  for i in countup(0, input.len - 1, 16):
    reg = input[i ..< i + 16]
    var s = input[i ..< i + 16]
    self.decryptBlock(s)
    s.`xor=`(iv)
    for j in 0 ..< 16: input[i + j] = s[j]
    iv = reg
  input.unpadPKCS7

proc decryptAES128CBC*(input, key, iv: seq[uint8]): seq[uint8] =
  doAssert key.len == 16
  doAssert iv.len == 16
  let cipher = newAES(key)
  var input = input
  cipher.decryptCBC(input, iv)
